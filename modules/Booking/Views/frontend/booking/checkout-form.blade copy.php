        <form class="form-horizontal" id="pembayaran" onsubmit="return submitForm();">
...
<script
    ...
    function submitForm() {
        $.post("{{ route('pembayaran.store') }}",
        {
            _method: 'POST',
            _token: '{{ csrf_token() }}',
            amount: $($booking->total).val(),
            note: $('input#zip_code').val(),
            pembayaran_type: $('input#state').val(),
            bayar_name: $('input#first_name').val(),
            bayar_email: $('input#email').val(),
        },
